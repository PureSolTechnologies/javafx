...
public class I18NFile {
	...
	BufferedReader reader = new BufferedReader(new FileReader(file, Charset.defaultCharset()));
	try {
	    String line;
    ...
    public static boolean isFinished(File file, Locale locale) throws IOException {
	return read(file).isTranslationFinished(locale);
    }

    public static void write(File file, MultiLanguageTranslations translations) throws IOException {
	try {
    ...
    public static MultiLanguageTranslations read(InputStream inputStream) throws IOException {
	try {
	    if (inputStream == null) {
		throw new IOException("Input stream was not available!");
	    }
	    JAXBContext context = JAXBContext.newInstance(MultiLanguageTranslations.class);
	    Unmarshaller unmarshaller = context.createUnmarshaller();
	    MultiLanguageTranslations translations = (MultiLanguageTranslations) unmarshaller.unmarshal(inputStream);
